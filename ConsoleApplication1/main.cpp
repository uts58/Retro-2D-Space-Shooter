//to-do
//individual audio done
//menu done
//difficulty done
//2 mode done
//high score for 2 mode done
//health added done
#include"pch.h"
#include<windows.h>
#include<stdio.h>
...
void DestroyAnimation(int x, int y, int radius)
{	//center cirle
...
void collisionship()
{
	for (int i = 0; i < NOF; i++)
	{
		...
		if ((ship.x1 < (e[i].x2 + e[i].w2)) && (e[i].x2 < ship.x1 + ship.w1) && ((ship.y1 + ship.h1 == e[i].y2 - e[i].h2)))
		{
			Health = Health - 5;
		}
	}
	for (int i = 0; i < NOF; i++)
	{
		...
		if ((ship.x1 < (e2[i].x2 + e2[i].w2)) && (e2[i].x2 < ship.x1 + ship.w1) && ((ship.y1 + ship.h1 == e2[i].y2 - e2[i].h2)))
		{
			Health = Health - 5;
		}
	}
	for (int i = 0; i < NOL; i++)
	{
		...
		if ((ship.x1 < (l[i].x2 + l[i].w2)) && (l[i].x2 < ship.x1 + ship.w1) && ((ship.y1 + ship.h1 == l[i].y2 - l[i].h2)))
		{
			if (Health < 20) {
				Health = Health + 5;
			}
...
void BulletsVsEnemyCollisionTest()
{
	for (int i = 0; i < NOB; i++)
	{
		for (int j = 0; j < NOF; j++) {
			...
			if ((e[j].x2 <= b[i].x) && (b[i].x <= (e[j].x2 + e[j].w2)) && (e[j].alive) && (e[j].y2 <= b[i].y + 20) && (b[i].y < 1050))
			{
	...
	for (int i = 0; i < NOB; i++)
	{
		...
		for (int j = 0; j < NOF; j++) {
			...
			if ((e2[j].x2 <= b[i].x) && (b[i].x <= (e2[j].x2 + e2[j].w2)) && (e2[j].alive) && (e2[j].y2 <= b[i].y + 20) && (b[i].y < 1050))
			{
